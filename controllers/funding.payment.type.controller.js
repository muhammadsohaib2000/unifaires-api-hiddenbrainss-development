const fundingPaymentType = require("../services/funding.payment.type.services");
const { useAsync } = require("../core");
const { JParser } = require("../core/core.utils");

exports.index = useAsync(async (req, res, next) => {
  try {
    const all = await fundingPaymentType.all();
...
exports.store = useAsync(async (req, res, next) => {
  try {
    const create = await fundingPaymentType.store(req);
...
exports.get_by_id = useAsync(async (req, res, next) => {
  try {
    ...
    const find = await fundingPaymentType.findOne(id);
...
exports.update = useAsync(async (req, res, next) => {
  try {
    ...
    if (find) {
      const update = await fundingPaymentType.update(id, req);
...
exports.destroy = useAsync(async (req, res, next) => {
  try {
    ...
    const destroy = await fundingPaymentType.destroy(id);
