...
exports.index = useAsync(async (req, res, next) => {
  try {
    const all = await courseCountryPricingServices.all(req);
...
exports.store = useAsync(async (req, res, next) => {
  try {
    ...
    const find = await courseCountryPricingServices.findBy({ country });

    if (!find) {
      // create

      const create = await courseCountryPricingServices.store(req);
...
exports.get_by_id = useAsync(async (req, res, next) => {
  try {
    ...
    const find = await courseCountryPricingServices.findOne(id);
...
exports.update = useAsync(async (req, res, next) => {
  try {
    ...
    const price = await courseCountryPricingServices.findOne(id);

    if (price) {
      const update = await courseCountryPricingServices.update(id, req);
...
exports.destroy = useAsync(async (req, res, next) => {
  try {
    ...
    if (price) {
      const destroy = await courseCountryPricingServices.destroy(id);
