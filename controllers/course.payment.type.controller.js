...
exports.index = useAsync(async (req, res, next) => {
  try {
    const all = await coursePaymentTypeServices.all();
...
exports.store = useAsync(async (req, res, next) => {
  try {
    const create = await coursePaymentTypeServices.store(req);
...
exports.get_by_id = useAsync(async (req, res, next) => {
  try {
    ...
    const find = await coursePaymentTypeServices.findOne(id);
...
exports.update = useAsync(async (req, res, next) => {
  try {
    ...
    if (find) {
      const update = await coursePaymentTypeServices.update(id, req);
...
exports.destroy = useAsync(async (req, res, next) => {
  try {
    ...
    const destroy = await coursePaymentTypeServices.destroy(id);
