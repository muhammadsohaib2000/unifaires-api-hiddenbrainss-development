...
exports.index = useAsync(async (req, res, next) => {
  try {
    const all = await jobBusinessPricingServices.all();
...
exports.store = useAsync(async (req, res, next) => {
  try {
    ...
    const find = await jobBusinessPricingServices.findBy({ businessId });
    ...
    const create = await jobBusinessPricingServices.store(req);
...
exports.get_by_id = useAsync(async (req, res, next) => {
  try {
    ...
    const find = await jobBusinessPricingServices.findOne(id);
...
exports.update = useAsync(async (req, res, next) => {
  try {
    ...
    const price = await jobBusinessPricingServices.findOne(id);

    if (!price) {
      return res.status(404).json(JParser("pricing not found", false, null));
    }

    const update = await jobBusinessPricingServices.update(id, req);
...
exports.destroy = useAsync(async (req, res, next) => {
  try {
    ...
    const destroy = await jobBusinessPricingServices.destroy(id);
